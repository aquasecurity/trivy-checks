...
const (
	...
	targetFile                 = "lib/cloud/allowed_actions_gen.rego"
	defaultParallel            = 10
)
...
func generateFile(path string, actions []string) error {
	...
	w := bufio.NewWriter(f)
	_, _ = w.WriteString(
		`# Code generated by cmd/aws_actions DO NOT EDIT.

package lib.aws.actions

import rego.v1

is_wildcard_denied(actions) if {
	some action in actions
	not action in actions_without_resource_level_support
}

actions_without_resource_level_support := {
`,
	)
...
func GenAllowedActions() error {
	...
	startTime := time.Now()
	defer func() {
		log.Printf("Parsing is completed: %.2f seconds", time.Since(startTime).Seconds())
	}()
