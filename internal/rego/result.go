...
var NewResultDecl = &rego.Function{
	Name: "result.new",
	Decl: types.NewFunction(types.Args(types.S, types.A), types.A),
}

func NewResultImpl(ctx rego.BuiltinContext, msg, cause *ast.Term) (*ast.Term, error) {
	...
	if term := input.Get(ast.StringTerm("parent")); term != nil {
		var err error
		metadata["parent"], err = NewResultImpl(ctx, nil, term)
		if err != nil {
			return nil, err
		}
	}
...
var IsManagedImpl = func(c rego.BuiltinContext, resource *ast.Term) (*ast.Term, error) {
	metadata, err := NewResultImpl(c, ast.StringTerm(""), resource)
	if err != nil {
		return nil, err
	}
